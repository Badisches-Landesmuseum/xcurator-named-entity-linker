...
class PlainTextListener(RabbitListener):

    def __init__(
            self,
            ned_analyser: NamedEntitiesAnalyser,
    ):
        self.name = "entities.text.analyser"
        self.exchange_name = 'assets'
        self.routing_keys = ['entities.analyse.text']
    ...
    async def on_message(self, proto: NamedEntitiesDetectionAction, message):
        ...
        try:
            entities = self.ned_analyser.analyse_text(proto.content)
        ...
        await self.convert_and_send("entities.text.analysed", detection_result, self.exchange_name)
